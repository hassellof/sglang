...
class HybridCacheController(BaseHiCacheController):
    ...
    def _l2_note_alloc_failure(self, pool, size, entry, alloc_host: bool) -> None:
        """Record which sidecar host pool refused an allocation.

        `write()` frees the anchor slots when this path is taken, so the
        anchor occupancy the caller logs is never the binding resource. Pure
        diagnostic state: nothing in the cache reads it for control flow.
        """
        try:
            avail = entry.host_pool.available_size() if alloc_host else None
        except Exception:  # noqa: BLE001 - diagnostic only, never fatal
            avail = None
        self._l2_last_alloc_failure = (str(pool.name), size, avail)
    ...
    def _resolve_pool_transfers_allocation(
        ...
        self._l2_last_alloc_failure = None
        if not extra_pools:
            return None
        ...
        for pool in extra_pools:
            ...
            if indices is None and evict_fn:
                ...
                indices = alloc_fn(size)
            if indices is None:
                self._l2_note_alloc_failure(pool, size, entry, alloc_host)
