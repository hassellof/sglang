...
class DeepseekV4ForCausalLMDSpark(nn.Module):
    ...
    def compute_confidence(
        ...
        *,
        ...
        # overrides it everywhere on the static path, but this confidence
        # path (compact / cap-accept only) still viewed by the NATIVE gamma,
        # so any configured gamma != native fails decode-graph capture with
        # a shape error. Derive the window from the tensor instead.
